<div>
    ...
    <x-ui.card flush>
        ...
        @else
                    ...
                    <tbody>
                        @foreach ($orders as $order)
                            ...
                            <tr wire:key="{{ $order->id }}">
                                ...
                                <td>
                                    <p class="text-ink-100">{{ $order->dropoffSnapshot()->contactName }}</p>
                                    <p class="text-2xs text-ink-400">{{ $order->dropoffSnapshot()->area }}</p>
                                </td>
                                ...
                                <td class="tnum text-end text-ink-400">
                                    {{ $order->created_at->translatedFormat('d M g:i A') }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
            ...
            @if ($orders->hasPages())
                <div class="border-t border-white/10 px-4 py-3">{{ $orders->links() }}</div>
            @endif
        @endif
    </x-ui.card>
</div>
