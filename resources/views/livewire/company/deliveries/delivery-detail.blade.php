...
<div wire:poll.10s
    ...
    <div class="grid gap-5 xl:grid-cols-3">
        <div class="space-y-5 xl:col-span-2">
            ...
            {{-- First, above the map. A recipient who has said something is
                 wrong outranks knowing where the bike is. --}}
            <livewire:shared.delivery-issues :delivery-id="$delivery->id"
                                             :key="'issues-'.$delivery->id" />
            ...
            @if ($canAssign)
                <x-ui.card :title="__('app.nav.riders')"
                    ...
                    @if ($this->hasOpenAssignment())
                        <div class="border-b border-white/5 bg-warn-500/10 px-4 py-2.5">
                            <p class="text-xs font-medium text-warn-200">
                            ...
                            @foreach ($this->availableRiders as $entry)
                                ...
                                <li class="flex items-center gap-3 px-4 py-3" wire:key="r-{{ $rider->id }}">
                                    <span class="flex size-8 shrink-0 items-center justify-center rounded-full
                                                 bg-white/[0.06] text-xs font-semibold text-ink-300">
                        ...
                        <li class="flex items-start gap-3 px-4 py-2.5">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                @if ($event->actor_label)
                                    <p class="text-2xs text-ink-400">{{ $event->actor_label }}</p>
                                @endif
                            </div>
            ...
            <x-ui.card :title="__('delivery.labels.price')">
                <dl class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        <dt class="text-ink-400">{{ __('delivery.labels.payout') }}</dt>
                    ...
                    <div class="flex justify-between">
                        ...
                        <dd class="tnum text-ink-100">{{ $delivery->riderPayout()->format() }}</dd>
                    </div>
                    ...
                    @if ($order->payment_type->requiresCollection())
                        <div class="flex justify-between rounded-md bg-warn-500/10 px-2 py-1.5">
                            <dt class="font-medium text-warn-200">{{ __('order.payment.cod') }}</dt>
                            <dd class="tnum font-semibold text-warn-200">
                    ...
                    <ul class="divide-y divide-white/5">
                        @foreach ($order->items as $item)
                            <li class="flex justify-between px-4 py-2 text-sm">
                                <span class="text-ink-100">{{ $item->name }}</span>
                                <span class="tnum text-ink-400">×{{ $item->quantity }}</span>
                            </li>
                        @endforeach
                    </ul>
                </x-ui.card>
